...
class simple_PPA:
    ...
    def single_member_update(self, data, i):
        ...
        elo = elo_sum@rem.T #How much do different candidates affect error
        ...
        pixel_spectrum = self.endmember_list[:, sor_eng[j]]
        ...
        while in_group:
            if j == data.shape[1] - 1:  # Break if at the last pixel
                break
            j += 1
            pixel_spectrum = self.endmember_list[:, sor_eng[j]]  # Get the next ranked spectrum
        ...
        if energies[sor_eng[j]] < 0:
            self.w[:, i] = self.endmember_list[:, sor_eng[j]]
        else:
            self.w[:, i] = self.endmember_list[:, sor_eng[2]]
